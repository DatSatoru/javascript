...
const selectorDiagnosis = document.querySelector('#formAge select');
...
function getDataForm(event) {
    ...
    let agemin = event.target.agemin.value;
    let agemax = event.target.agemax.value;
    let diagnostico = event.target.diagnosis.value;
    let patientsResult = []
    ...
    if (diagnostico === "") {
        patientsResult = filterByAge(pacientes, Number(agemin), Number(agemax))
    } else if (agemin === "" && agemax === "") {
        patientsResult = filterByDiagnosis(pacientes, diagnostico);
    } else {
        const patientsAge = filterByAge(pacientes, Number(agemin), Number(agemax));
        patientsResult = filterByDiagnosis(patientsAge, diagnostico)
    }
 ...
 * 
 * recibo una lista
 * recibo el selector donde tengo que pintar las opciones
 */
